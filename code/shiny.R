...
ui <- fluidPage(
  ...
  tabsetPanel(type = "tabs",
              ...
              tabPanel("Random Forest", mainPanel(plotOutput("rf_tree"))),
              tabPanel("Xgboost", mainPanel( tableOutput('xgboost')))
  )
)
...
server <- function(input, output) {
  ...
  output$featurePlot <- renderPlot({
    ...
    if(input$feature == 'Gender'){
      ...
      print(plot_grid(gender_bar, gender_pie, align='v', nrow=2, rel_heights = c(2, 1)))
    }
  ...
  ######################
  ### random forest tree
  rftree <- ctree(Attrition_Flag ~ ., data=train_cl2, control=ctree_control(maxdepth = 3) )
  output$rf_tree <- renderPlot({
    print("random forest")
    plot(rftree, type='simple')
  })

  
  ###########
  ### xgboost
  dummy <- dummyVars(" ~.", data=train_cl1[,-1])
  train.x <- data.frame(predict(dummy, newdata = train_cl2[,-1]))
  train.y <- ifelse(train_cl2$Attrition_Flag=="Attrited Customer", 1, 0)
  
  xg.model <- xgboost(data = as.matrix(train.x), label=as.matrix(train.y), 
                   max.depth = 6, eta = 1, nthread = 2, eval_metric = "logloss",
                   nrounds = 9, objective = "binary:logistic")
  # get xgb importance 
  xgb_imprtance <- xgb.importance(colnames(as.matrix(train.x)), model = xg.model)
  # plot feature importance
  ## measure: can be "Frequency", "Gain", or "Cover"
  output$xgboost <- renderTable({
    p <- xgb.plot.importance(xgb_imprtance, rel_to_first = TRUE, 
                            measure= "Frequency", xlab = "Relative importance")
    print(p)
  })
  # output$xgboost_tree <- renderPlot({
